...
#' figureS_sanity
#'
#' figureS1: generate model parameters from the prior range, simulate dataset, and see if the model can recover them accurately
...
#' figureS_sanity
#'
#' figureS4: generate synthetic datasets to demonstrate how the mixture-BDT only allows for horizontal shift by changing rho
#'
#' @param sigma_p numeric, perceptual noise
#' @param rho numeric, utility function exponent
#'
#' @return ggplot object
figureS_bdt_shift = function(sigma_p = 0.3, rho = 1){
  # figureS4: generate synthetic datasets to demonstrate how the mixture-BDT only allows for horizontal shift by changing rho
  df = sim_subject(params_list = list(sigma = sigma_p, rho = rho, omega_1 = 0.8, omega_2 = 0.1, omega_3 = 0.1))
...
#' figureS_sigmoid
#'
#' figureS5: plots sigmoid prediction overlayed with binned subject choice data
#'
#' @param animal subjid from the \code{good_list}
#'
#' @return ggplot object
figureS_sigmoid = function(animal = 2143){
  # figureS5: plots sigmoid prediction overlayed with binned subject choice data
  draws_df = read.csv('csv/sigmoid_fits.csv') %>% filter(subjid == animal)
  subj_df = read.csv('csv/preprocessed_good_trials.csv') %>% filter(subjid == animal)
  sim_df = data.frame()
  for (flash_side in c('Low', 'None', 'High')){
    this_draws_df = draws_df %>% filter(better_side_s == flash_side)
    sim_df = rbind(sim_df, sigmoid_agent(w1 = this_draws_df$w1, w2 = this_draws_df$w2, x0 = this_draws_df$x0, b = this_draws_df$b) %>%
                     mutate(subjid = animal, better_side_s = flash_side))
  }
