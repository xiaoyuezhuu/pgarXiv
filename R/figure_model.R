...
#'
#' figure5a: plot prediction ribbons from the mixture-BDT model overlayed with subject binned choice data
#'
#' @param animal subjid from the \code{good_list}
#'
#' @return ggplot object
figure_model_prediction = function(animal = 2143){
  # figure5a: plot prediction ribbons from the mixture-BDT model overlayed with subject binned choice data
  subj_df = read.csv('csv/preprocessed_good_trials.csv') %>% filter(better_side != 2) %>% filter(subjid == animal)
...
#' figure_model_pairs
#'
#' figure5b: aggregated pairs plot using fits from the mixture-BDT model, summarise the fit for each parameter
#'
#' @param control_ticks Boolean, if TRUE the lower and upper limits of ticks are set to specified values
#'
#' @return ggplot object
...
#' figure_model_sigmoid
#'
#' figure6: bar plot showing the change in the four-parameter sigmoid terms in different flash conditions
#'
#' @return ggplot object
