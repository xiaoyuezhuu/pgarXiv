...
#'
#' figure3a: example psychometric curve from one example animal
#' @param animal subjid from the \code{good_list}
#' @param show_sessions Boolean, if TRUE all the session lines will be shown
#' @param n_sessions integer, the number of sessions to draw randomly from this subject's dataset
#'
...
#' figure_behavior_violin
#'
#' figure3b: violin plot showing the distribution of % choosing high by condition across sessions
#' @param animal subjid from the \code{good_list}
#'
#' @return ggplot object
...
#' figure_behavior_difficulty_schematic
#'
#' figure3d: plots a schematic bell curve for easy - medium - hard illustration
#'
#' @return ggplot object
...
#'
#' figure3c: psychometric curves for the population
#'
#' @return ggplot object
figure_behavior_population = function(){
  ...
  p = ggplot() + theme_classic(base_size = BASE_SIZE) +
    ...
    xlab(expression(log2(Frequency))) + ylab("P(Chose High)") +
    scale_color_manual(values = c('azure4','#81B628','#F37970'), name = 'Flash on') +
    theme(legend.position = 'none')
...
#' figure_behavior_population
#'
#' figure3f: plot psychometric curves based on previous trial's outcome
#'
#' @param animal subjid from the \code{good_list}
#'
#' @return ggplot object
figure_behavior_history = function(animal = 2143){
  # figure3f: plot psychometric curves based on previous trial's outcome
  subj_df = read.csv('csv/preprocessed_good_trials.csv') %>% filter(subjid == animal) %>%
    mutate(prev_flash = lag(better_side_s), prev_correct = lag(correct_choice), prev_choice = lag(chose_high),
  ...
  subj_df$prev_outcome = factor(subj_df$prev_outcome, c('Low win', 'Low lose', 'High lose', 'High win'))
